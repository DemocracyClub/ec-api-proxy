...
class APIKeyForLogging:
    ...
    def prefix(self):
        if self.dc_environment == "production":
            return "api-users/ec-api/latest/"

        if self.dc_environment in ("development", "staging"):
            return f"api-users/ec-api/{self.dc_environment}/latest/"

        return "api-users/ec-api/local-dev/latest/"
    ...
    def from_django_model(cls, api_key_model):
        """
        :type api_key_model: ec_api.apps.users.models.APIKey
        """
